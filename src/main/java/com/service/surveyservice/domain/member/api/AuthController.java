...
public class AuthController {
    ...
    public ResponseEntity<MailDTO.EmailSentDTO> mailConfirm(@RequestBody MailDTO.ConfirmEmailDTO confirmEmailDTO) throws Exception {
        emailCertificationService.sendSimpleMessage(confirmEmailDTO.getEmail());
        return new ResponseEntity<>(MailDTO.EmailSentDTO.builder().email(confirmEmailDTO.getEmail()).success(true).build(), HttpStatus.OK);
    }

    // 코드 인증 요청 : 이메일 인증에서 인증 코드 생성과 동시에 이메일을 발송하고, 이 api로 코드가 일치한지 확인한다.
    @GetMapping(value = "/auth/codeConfirm")
    public ResponseEntity<MailDTO.CodeConfirmDTO> codeConfirm(@RequestBody MailDTO.EmailConfirmCodeDTO emailConfirmCodeDTO) {
    ...
    public ResponseEntity<MailDTO.EncryptEmailDTO> findEmail(@RequestBody MailDTO.FindEmailDTO findEmailDTO) {
        MailDTO.EncryptEmailDTO userEmail = memberService.findUserEmail(findEmailDTO);
